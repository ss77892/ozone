...
public class OzoneCryptoInputStream extends CryptoInputStream
   ...
   * the middle of a positioned read and see (or undo) the cursor move that read does. This one is the
   * sequential read MultipartInputStream uses for a part, so it races with the positioned read of the
   * multipart stream, which is intentionally lock-free.
   */
  @Override
  ...
  public synchronized int read(long position, ByteBuffer dst) throws IOException {
    ...
    try {
      ...
      while (totalReadLen < numBytesToRead) {
        // read(byte[], int, int) returns at most one Crypto buffer worth of data per call.
        final int numBytesRead = read(buffer, 0, Math.min(buffer.length, numBytesToRead - totalReadLen));
        if (numBytesRead <= 0) {
          break;
        }
  ...
  public void readFully(long position, ByteBuffer dst) throws IOException {
    ...
    while (dst.hasRemaining()) {
      final int numBytesRead = read(position + done, dst);
      if (numBytesRead == EOF) {
  ...
  public boolean hasCapability(String capability) {
    if (StreamCapabilities.PREADBYTEBUFFER.equals(StringUtils.toLowerCase(capability))) {
      // The positioned read of this stream moves the cursor and is serialized against the other reads.
      return false;
    }
